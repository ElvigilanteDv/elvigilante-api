...
class Pinterest {
  ...
  async download({ url: pinUrl }) {
    if (!pinUrl) {
      return { status: false, code: 400, result: { message: "Pin URL cannot be empty." } };
    }
    if (!this.isUrl(pinUrl)) {
      return { status: false, code: 400, result: { message: "This is not a valid URL." } };
    }
    try {
      const finalUrl = await this.followRedirects(pinUrl, 2);
      if (!this.isPin(finalUrl)) {
        return { status: false, code: 400, result: { message: "This is not a valid Pinterest link." } };
      }
      const pinId = finalUrl.split("/pin/")[1]?.split("/")[0]?.split("?")[0];
      if (!pinId) {
        return { status: false, code: 400, result: { message: "Could not extract pin ID from URL." } };
      }
      if (!this.cookies) {
      ...
      if (!data.resource_response.data) {
        return { status: false, code: 404, result: { message: "Pin not found." } };
      }
...
router.get('/', async (req, res) => {
    ...
    if (!url) {
        return res.status(400).json({ status: false, error: 'Debes proporcionar una URL de Pinterest.' });
    }
